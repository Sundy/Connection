...
test('plan confirmation layout separates read-only existing work from deletable additions', () => {
  ...
  const blockerStart = markup.indexOf('draft.confirmation_blockers')
  ...
  assert.doesNotMatch(markup, /安排预览/)
  assert.doesNotMatch(markup, /draft\.daily_preview/)
  assert.doesNotMatch(markup, /对应本次新增作业/)

  const existingSection = markup.slice(existingStart, newHeading)
  const newSection = markup.slice(newStart, blockerStart)
